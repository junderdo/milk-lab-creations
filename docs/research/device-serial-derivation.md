...
- **The deployed web client already tolerates it.** `parseCapability`
  ...
  firmware change to land safely — only to *use* the new field.
...
administered space rather than a locally administered one. That is a statement about *which address
space the value comes from*, and it carries the strong implication that Espressif does not reissue
...
Both are wrong for ESP32-C2. BLK1 on C2 is the user-data block that holds the *custom* MAC — as the
...
and it has been carried forward as far as v5.5.2. It does not cast doubt on the *base* MAC — only on
...
clear that re-MACing is an *expected* C2 workflow in a way it is not on C3/S3. That is inference from
...
| `esp_read_mac(mac, ESP_MAC_BASE)` | the base MAC — factory MAC *unless* overridden | **Yes** | No |
...
**A correction worth recording, because the obvious reading is wrong.** ESP32-C2 is *not* a chip
...
among `build_component_paths` — but that is a *private* requirement of a component we merely depend
...
`port/include` *is* a public `INCLUDE_DIRS` entry of the mbedtls component, so it compiles. But there
...
1. **It matches the input width.** The message being hashed is a 48-bit MAC. The input space *is*
...
One caveat worth naming: `esp_read_mac(mac, ESP_MAC_BT)` is *also* safe to call before BLE init — the
...
1. ~~**The watch is the third client.**~~ *Resolved.* `AnimationStoreService::onCapabilityResponse`
...
6. **Should the serial also be advertised?** Every unit currently advertises the identical name
   `ROBO_CAT_EARS`, so a user with two pairs cannot tell them apart in the Chrome chooser *before*
