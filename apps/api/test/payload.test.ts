...
describe("packWireFormat", () => {
  it("packs a single keyframe to the firmware byte layout", () => {
    ...
    expect([...wire]).toEqual([
      1, // keyframe_count
      0x12, 0x34, // time_ms big-endian
      10, 20, 30, 40, // left_azi, left_lat, right_azi, right_lat
      1, 3, // ease_in_type, ease_out_type
      0x01, 0x02, // ease_in_ms
      0x0a, 0x0b, // ease_out_ms
    ]);
  });
...
describe("derivedScalars", () => {
  it("takes duration from the last keyframe", () => {
    const scalars = derivedScalars({
      schemaVersion: 1,
      keyframes: [
        { timeMs: 0, angles: [0, 0, 0, 0], easeInType: 0, easeOutType: 0, easeInMs: 0, easeOutMs: 0 },
        { timeMs: 2500, angles: [0, 0, 0, 0], easeInType: 0, easeOutType: 0, easeInMs: 0, easeOutMs: 0 },
      ],
    });
