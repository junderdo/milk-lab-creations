...
import { keyframeWireSize, packWireFormat } from "../src/wire-format.ts";
...
describe("packWireFormat", () => {
  ...
  it("hits the firmware max serialized size at 64 keyframes", () => {
    ...
    const wire = packWireFormat({ keyframes });
    // CUSTOM_ANIMATION_MAX_SERIALIZED_SIZE = 1 + 64 * 12 = 769
    expect(wire.length).toBe(769);
    expect(keyframeWireSize(4)).toBe(12);
  });

  it("refuses an animation with no keyframes rather than packing a lie", () => {
    expect(() => packWireFormat({ keyframes: [] })).toThrow();
  });
});
